		...
		this.__loader={
			...
			loadApp:function(data){
				...
				if(!data.error){
					...
					module.__loader.loadFonts(data);
					if(data.combined){
					...
					_alert('Im sorry, you are not authorized to use this app');
				}
			},
			loadStaticFiles:function(data){
				if(data.static && data.static.length) for (var i = 0; i <= data.static.length - 1; i++) {
					var js=data.static[i]+'?t='+new Date().getTime();
					if(isPhoneGap()&&js.indexOf('http')==-1) js=module.__loader.vars.siteurl+js;
					$('head').append('<script src="'+js+'" ></script>');
		    ...
		    initApp:function(count){
		    	if(window.app&&module.__loader.templates&&module.jsloaded&&module.templatesloaded){
