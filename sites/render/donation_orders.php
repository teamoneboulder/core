<?php
	...
	$fundraiser_id=$r['path'][3];
	db2::findOne('nectar','fundraiser',array('id'=>$fundraiser_id));
	include_once(ROOT.'/api/class/fundraiser.php');
	include_once(ROOT.'/sites/nectar/api.php');
	$data=FUNDRAISER::orders(array(
		'auth'=>array(
			'scope'=>'*'
		),
		'qs'=>array(
...
<html>
	<head>
		...
		<script>
			...
			window._templates='<?php echo phi::miniFile(ROOT,["/sites/render/donation_orders.templates"]);?>';
			function onload(){
