...
ss.obj2string = function( obj, prefix ) {
    "use strict";
    //if($.param) return $.param(obj);//allows deeper object
    var str = [];
...
ss.getExt = function( file ) {
    "use strict";
    var twoPartExts=['tar.gz'];
    for (var i = 0; i < twoPartExts.length; i++) {//add ability for two part extension types, eg tar.gz
       var text=twoPartExts[i];
       if(file.indexOf(text)>=0) return text;
    }
...
ss.SimpleUpload = function( options ) {
    ...
    this._opts = {
        ...
        onQueue: function( queueobj ) {},
        onSubmit: function( fileobj ) {},
        onProgress: function( pct ,fileobj,stats) {},
    ...
    this._btns = [];
    this.version=1;
    this.addBlob=function(blob,format){
        var obj={
            id: ss.getUID(),
            file: blob,
            name: 'file.'+format,
        ...
        this._opts.onQueue.call( this, obj);
        this._queue.push(obj);
    }
    this.processUpload=function(post_options){
        for (var key in post_options) {
            this._opts.data[key]=post_options[key];
        }
        this._initUpload( this._queue[0],1);
    }
    this.clear=function(){
        if(this._queue[0]) this.removeCurrent( this._queue[0].id );
    }
    ...
    */
    ...
    rerouteClicks: function( elem ) {
        ...
        elem.off = ss.addEvent( elem, 'mouseover', function() {
            ...
            self._input.style.width=self._input.parentNode.style.width;
        });
...
ss.IframeUpload = {
    ...
    */
    ...
    _addFiles: function( file ) {
        if(this._opts.uploadOnClick){//before adding file, clear out the old one!
            if(this._queue[0]){
                this.removeCurrent( this._queue[0].id );
            }
        }
        ...
        self._detachEvents[ iframe.id ] = ss.addEvent( iframe, 'load', function() {
            ...
            if ( abortBtn ) {
                cancel = function() {
                    ...
                    opts.onAbort.call( self, fileObj);
...
ss.XhrUpload = {
    ...
    _addFiles: function( files ) {
        if(this._opts.uploadOnClick){//before adding file, clear out the old one!
            if(this._queue[0]){
                console.log('CLEAR: '+this._queue[0].id);
                this.removeCurrent( this._queue[0].id );
            }
        }
    ...
    */
    _uploadXhr: function( fileObj, url, params, headers, sizeBox, progBar, progBox, pctBox, abortBtn, removeAbort ) {
        ...
        if ( abortBtn ) {
            ...
            ss.addEvent( abortBtn, 'click', function(){
                if(opts.handleAbortClick){
                    opts.handleAbortClick(function(){
                        cancel();
                    })
                }else{
                    cancel();
        ...
        ss.addEvent( xhr.upload, 'progress', function( event ) {
            if ( event.lengthComputable ) {
                ...
                var stats={
                    ...
                    avgSpeed:'',//mb/s
                    elapsedTime:'',//seconds
                    loaded:event.loaded/(1024*1024),//mb
                    total:event.total/(1024*1024),//mb
                    remaining:''//mb
                }
                stats.remaining=stats.total-stats.loaded;
                var dt=(new Date().getTime()-fileObj.startTime)/1000;
                stats.elapsedTime=dt;
                stats.avgSpeed=(stats.loaded/stats.elapsedTime);
                stats.timeRemaining=stats.remaining/stats.avgSpeed;
                if(fileObj.last){
                    var dbits=stats.loaded-fileObj.last.loaded;
                    var dtime=dt-fileObj.last.time;
                    stats.speed=dbits/dtime;
                }
                fileObj.last={
                    loaded:stats.loaded,
                    time:dt
                }
                opts.onProgress.call( self, pct,fileObj,stats);
        ...
        fileObj.startTime=new Date().getTime();
        opts.onProgress.call( this, 0 ,fileObj);
        ...
        if ( opts.multipart === true ) {
            ...
            for ( var prop in params ) {
                if ( params.hasOwnProperty( prop ) ) {
                    ...
                    if(typeof params[prop]=='object'&&!params[prop].length){//not array
                        formData.append(prop,JSON.stringify(params[prop]));
                    }else{
    ...
    _initUpload: function( fileObj ,force) {
        "use strict";
        if(this._opts.previewOnly&&!force){
            return false;
        }
        var params = {},
            headers = {},
        ...
        if(window.$) params=$.extend(true,{},params,this._opts.data)
        else ss.extendObj( params, this._opts.data );
        // Build query string while preserving any existing parameters
        url = this._opts.noParams === true ?
                this._opts.url :
                this._opts.url + ( ( this._opts.url.indexOf( '?' ) > -1 ) ? '&' : '?' ) + ss.obj2string( params );
        //create an abort btn for individual uploads
        $('body').append('<div id="'+fileObj.id+'_abort"></div>');
        this._uploadXhr( fileObj, url, params, headers, this._sizeBox, this._progBar, this._progBox, this._pctBox, document.getElementById(fileObj.id+'_abort'), this._removeAbort );
...
ss.extendObj( ss.SimpleUpload.prototype, {
    ...
    _createInput: function() {
        ...
        if(self._opts.parent) self._opts.parent.appendChild( div );
    ...
    */
    _errorFinish: function( fileObj, status, statusText, response, errorType, progBox, sizeBox, pctBox, abortBtn, removeAbort ) {
        ...
        this._opts.onError.call( this, statusText, fileObj );
    ...
    */
    _finish: function( fileObj, status, statusText, response, sizeBox, progBox, pctBox, abortBtn, removeAbort ) {
        ...
        this._opts.onComplete.call( this, fileObj, response);
        this._last( sizeBox, progBox, pctBox, abortBtn, removeAbort );
