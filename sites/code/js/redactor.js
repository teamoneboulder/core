			...
			return {
				start: function()
				{
					...
					else if (this.opts.type === 'div')
					{
						...
						if (html === '')
						{
							this.$editor.html(this.opts.emptyHtml);//here
						}
						this.observe.load();
					}
		...
		core: function()
		{
			return {
				...
				callback: function(type, e, data)
				{
					...
					{
						if(type=='change'){
							var scrollEle=this.$element.parents('.scrollingarea');
							if(scrollEle.length){//inside a scroller
								if(scrollEle[0].scrollTop!=0) scrollEle[0].scrollTop=0
							}
						}
			...
			return {
				...
				send: function(formData, e)
				{
					...
					if(this.opts.postData){
						$.each(this.opts.postData,function(name,value){
							formData.append(name,value);
						})
						//formData=$.param(this.opts.postData)
					}
					xhr.send(formData);
				},
				onDrag: function(e)
