<?php
	class filter_content{
		...
		public static function update($r){
			...
			db2::update('nectar','content_filter',array('id'=>$r['auth']['uid']),array('$set'=>$set));
		...
		public static function delete($r){
			...
			$k='filters.list.'.$id;
			$k2='filters.order';
			db2::update('nectar','content_filter',array('id'=>$r['auth']['uid']),array('$unset'=>array($k=>1),'$pullAll'=>array($k2=>array($id))));
			return array('success'=>true);
		}
		public static function order($r){
			phi::ensure($r,array('order'));
			$order=$r['qs']['order'];
			db2::update('nectar','content_filter',array('id'=>$r['auth']['uid']),array('$set'=>array('filters.order'=>$order)));
			return array('success'=>true);
		}
		public static function save($r){
			...
			db2::update('nectar','content_filter',array('id'=>$r['auth']['uid']),array('$set'=>$set,'$addToSet'=>array('filters.order'=>array('$each'=>array($id)))),array('upsert'=>true));
			return array('success'=>true);
		}
	}
?>