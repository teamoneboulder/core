...
$.Redactor.prototype.groupup_image = function()
{
    return {
        init: function ()
        {
            var self=this;
            var button = this.button.add('groupup_image', 'Image');
            this.button.setIcon(button, '<i class="re-icon-image"></i>');

            this.$toolbar.find('.re-groupup_image').bindUploader({
                apiurl:app.uploadurl,
                data:{
                    sizes:['full','small'],
                    path:'/img/',
                    site:'nectar'
                },//$.extend(true,{},{},self.opts.postData),
                growl:true,
                ...
                onPreviewReady:function(data,obj){//render preview
                    // console.log(obj)
                    // self.renderPreviewImage(ele,obj.id,data);
                    console.log('preview ready!')
                },
                onProgress:function(p,obj){
                    //self.setUploadProgress(ele,obj.id,p);
                    console.log('onprogress ['+p+']!')
                },
                onUploadStart:function(){
                    ele.find('.uploadprogressbox').show();
                },
                onSuccess:function(obj,resp,id){
                    console.log(resp)
                    var img={
                        path:resp.path,
                        ext:resp.ext
                    ...
                    console.log(img)
                    var json={
                        url:modules.tools.getImg(img,'small')
                    }
                    console.log(json)
                    //self.image.insert(json);
                    $img=$('<figure><img src="'+modules.tools.getImg(img,'small')+'" alt=""/></figure>');
                    var obj=$(self.insert.node($img));
                    self.caret.after(obj);
                    self.code.sync();//update
                }
            });
            // window.app.bindUploader(this.$toolbar.find('.re-groupup_image'),{
            //     data:self.opts.postData
            // },function(resp){
            //     var img={
            //         path:resp.path,
            //         ext:resp.ext
...
$.Redactor.prototype.style_button = function()
{
    return {
        init: function ()
        {
            var self=this;
            var button = this.button.add('style_button', 'Button');
            var dropdown = {};

            dropdown[0] = { title: 'Blue Button', func:this.style_button.insert, args: ['blue'] };
            //var button = this.button.addAfter('format', 'inline', this.lang.get('style'));
            this.button.addDropdown(button, dropdown);
            self.bindButtons();
        },
        ...
        insert:function(args){
            //console.log(args)
            var self=this;
            $btn=$('<span class="redactor_button" style="padding:2px 5px;color:white;background:#0193be">Button</span>');
            var obj=$(self.insert.node($btn));
            self.caret.after(obj);
            self.code.sync();//update
            self.bindButtons();
        }
...
$.Redactor.prototype.groupup_file = function()
{
    return {
        init: function ()
        {
            var self=this;
            var button = this.button.add('groupup_file', 'File');
            window.app.bindFileUploader(this.$toolbar.find('.re-groupup_file'),{
                data:self.opts.fileUpload.postData
            },function(resp){
                self.groupup_file.callback(resp);
            });
            self.groupup_file.renderFiles();
        },
        callback:function(resp,redactor){
            var self=this;
            var obj={
                _id:app.getuuid('file'),
                url:resp.url,
                name:resp.name,
                type:resp.type
            };
            //add to po
            if(!self.opts.dataObject.files) self.opts.dataObject.files=[];
            self.opts.dataObject.files.push(obj);
            self.groupup_file.renderFiles();
        },
        renderFiles:function(){
            var self=this;
            var files=self.opts.dataObject.files;
            var ele=self.$element;
            var pele=ele.parent();
            if(pele.find('.fileuploadarea').length) pele.find('.fileuploadarea').remove();
            if(files&&files.length){
                pele.render({
                    ...
                    bindings:[{
                        ...
                        selector:'.x_remove',
                        binding:function(){
                            //change file name or delete
                            var id=$(this).attr('data-id');
                            var k=app.getIndexByKey(self.opts.dataObject.files,id,'_id');
                            var obj=app.getByKey(self.opts.dataObject.files,id,'_id');
                            app.confirm({
                                icon:'icon-trash',
                                text:'<div style="font-size:16px;padding-bottom:10px;">Remove this attachment <b>'+obj.name+'</b>?</div>',
                                buttons:[{
                                    btext:'Yes',
                                ...
                                cb:function(confirm){
                                    if(confirm){
                                        self.opts.dataObject.files.splice(k,1);
                                        self.groupup_file.renderFiles();
                                    }
                                }
                            });
...
(function($)
{
    $.Redactor.prototype.video = function()
    {
        return {
            ...
            init: function()
            {
                var button = this.button.add('groupup_image', '<i class="icon-video"></i> Video');
    ...
    $.Redactor.prototype.fontcolor = function()
    {
        return {
            init: function()
            {
                ...
                var $button = this.button.add('fontcolor', 'Font Color');
...
;modules.redactor=function(ele,options){
    var self=this;
    self.ele=ele;
    var ropts=$.extend(true,{
        cleanOnPaste: true,
        plugins: ['groupup_image','fontcolor','fontsize'],
    ...
    ropts.callbacks.change=function(){
        var rself=this;
        if(self.rsto) clearTimeout(self.rsto)
        self.rsto=setTimeout(function(){
            self.rsto=false;
            ropts.onUpdate(rself.code.get());
        },100);
    }
    ropts.callbacks.imageUpload=function(){
        var rself=this;
        if(self.rsto) clearTimeout(self.rsto)
        self.rsto=setTimeout(function(){
            self.rsto=false;
    ...
    if(ele){
        ele.redactor(ropts).addClass('editablearea');
        //return redactor object
        return ele.redactor('core.object');
    }else alert('no ele for redactor.init');
}