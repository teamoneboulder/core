...
$R.add('class', 'cleaner.figure', {
    ...
    _convertWidget: function(node)
    {
        ...
        if(!$node.attr('data-redactor-type')){//EDIT
...
$R.add('service', 'autoparser', {
    ...
    parse: function(html)
    {
        ...
        // variables
        if (this.opts.autoparseVariables)
        {
            html = this._formatVariables(html);
        }
        // restore
    ...
    _formatVariables: function(content, enter)
    {
        if(this.opts.variables){
            for (var i = 0; i < this.opts.variables.length; i++) {
                var v=this.opts.variables[i];
                var matches = content.getMatches(v);
                if(matches&&matches.length){
                    for (var ti = matches.length - 1; ti >= 0; ti--) {//work in reverse!
                        var match=matches[ti];
                        if(match.start>0){
                            var beforechar=content[match.start-1];
                            if(beforechar!='>'){//its not wrapped, wrap it!
                                content=content.replaceString(match.start,match.end,'<span data-redactor-type="variable" class="redactor-component">'+v+'</span>');
                            }
                        }
                    }
                }
...
$R.add('module', 'contextbar', {
    ...
    _buildPosition: function(e, $el, position)
    {
        ...
        var offset = (isTarget) ? $($el.nodes[0]).relativeOffset($(this.$target.nodes[0])) : $el.offset();
        var width = $el.width();
        ...
        var eleoffset=$($el.nodes[0]).offset();
        var leftFix = (isTarget&&false) ? targetOffset.left : 0;
        var topFix = (isTarget&&false) ? targetOffset.top : 0;

        if (!position)
        {
            var difftop=e.clientY-eleoffset.top;
            var diffleft=e.clientX-eleoffset.left;
            top=offset.top+difftop;
            left=offset.left+diffleft;
        }
...
$R.add('class', 'toolbar.fixed', {
    ...
    _doFixed: function()
    {
        ...
        var toleranceEnd = 60;
        var containerOffset = (this.toolbar.isTarget()||true) ? $($container.nodes[0]).relativeOffset($(this.$fixedTarget.nodes[0])) : $container.offset();
        if(!containerOffset||!containerOffset.top) return false;
        ...
        var top = (!this.toolbar.isTarget()) ? 0 : this.$fixedTarget.offset().top;
        //this.$fixedTarget, scroller
        //$container, redactor box
        //$wrapper, toolbar wrapper
        //console.log(containerOffset,scrollOffset,boxOffset);
        if (scrollOffset > boxOffset && scrollOffset < boxEnd)
        {
            var position = (this.detector.isDesktop()) ? 'fixed' : 'absolute';
            if(this.detector.isDesktop()&&this.opts.getTransformOffset&&this.opts.getTransformOffset()&&this.opts.getTransformOffset().top){
                top=top-this.opts.getTransformOffset().top
            }
            //position='fixed';
            top = (this.detector.isDesktop()) ? top : (scrollOffset - boxOffset);
            if (this.detector.isMobile()&&false)
            {
            ...
            else
            {
                ...
                //$($toolbar.nodes[0]).parent().css({zIndex:10})
            }
    ...
    onmodal: {
        link: {
            open: function($modal, $form)
            {
                ...
                var p=$($modal.nodes[0]).parent();
                p.append('<div class="notonweb" style="height:60vh"></div>');
               this.form=new modules.formbuilder({
                    ele:$($modal.nodes[0]).find('.formcontent'),
                    current:linkData,//passed as a refernce
                    inline:true,
                    scroller:new modules.scroller(p,{hasInput:true}),
                    mobileLayout:true,
                    schema:{
                        order:['url','text'],
                        fields:{
            ...
            {
                var data = this.form.getCurrent();
                if(this.form.getMissing().length){
        ...
        link_old: {
            open: function($modal, $form)
            {
                ...
                if(!modules.tools.isWebLayout()){
                    $($modal.nodes[0]).wrap('<div style="min-height:150vh;position:relative;"></div>');
                }
            },
            opened: function($modal, $form)
            {
...
$R.add('class', 'input.delete', {
    ...
    _removeUnwantedStyles: function()
    {
        ...
        setTimeout(function()
        {
            ...
            $tags.not('img, figure, iframe, [data-redactor-style-cache], [data-redactor-span], [data-redactor-link]').removeAttr('style');
...
$R.add('class', 'image.component', {
    ...
    _set_align: function(align)
    {
        ...
        else
        {
            ...
            {
                case 'left':
                    imageMargin = '20px auto 20px 0px';
                break;
                case 'right':
                    imageMargin = '20px 0px 20px auto';
                break;
                case 'center':
                    imageMargin = '20px auto 20px auto';
                break;
            }
            //$el.css({ 'float': imageFloat, maxWidth: width + 'px', 'margin': imageMargin, 'text-align': textAlign });
            $el.css({maxWidth: width + 'px', 'margin': imageMargin });
...
(function($R)
{
    $R.add('plugin', 'imagemanager_nectar', {
        translations: {
            en: {
                "choose": "Choose"
            }
        ...
        start:function(){
            var $button = this.toolbar.addButton('image', {
                title:'Add Image'
            });
            $button.setIcon('<i class="re-icon-image"></i>');
            var ele=$($button.nodes[0]);
            var tself=this;
            new modules.cropuploader($.extend(true,{
                btn:ele,
                onSuccess:function(data,cself){
                    cself.saving();
                    cself.destroy();
                    // var newdata=[{
                    //     "url": modules.tools.getImg(data,'small'),
                    //     "id": Math.uuid(16),
                    //     "title": "Image"
                    // }];
                    // console.log(newdata);
                    tself.app.api('module.image.insert', { image: {
                        "url": modules.tools.getImg(data,'small'),
                        "id": Math.uuid(16),
                        "title": "Image"
                    }});
                    //tself.app.api('plugin.imagemanager_nectar._parse', newdata);
                    //tself._parse(newdata,tself);
                }
            },{
                directUpload:true,
                sizes:['small','full'],
        ...
        _parse: function(data,tself)
        {
            ...
            for (var key in data)
            {
                ...
                $img.attr('src', url);
                $img.attr('data-image', Math.uuid(10));
                $img.attr('data-params', encodeURI(JSON.stringify(obj)));
                $img.css({
                    width: '96px',
                    height: '72px',
...
$R.add('plugin', 'button', {
    modals: {
        ...
        'button2':
            '<form action=""> \
                ...
                <div class="form-item"> \
                    <label for="modal-link-background">Background Color</label> \
                    <input type="text" id="modal-link-background" name="background"> \
                </div> \
                <div class="form-item"> \
                    <label for="modal-link-color">Text Color</label> \
                    <input type="text" id="modal-link-color" name="color"> \
                </div> \
                <div class="form-item"> \
                    <label for="modal-link-align">Align</label> \
                    <select id="modal-link-align" name="align"><option value="center">Center</option><option value="left">Left</option><option value="right">Right</option></select> \
                </div> \
            </form>'
    },
    init: function(app)
    ...
    start: function()
        {          
            var $button = this.toolbar.addButton('button', { title: 'Button',api: 'plugin.button.handleclick'});
            $button.setIcon('<i class="icon-grid"></i>');
        },
        ...
        button: {
            open: function($modal, $form)
            {
               ...
               this.form=new modules.formbuilder({
                    ...
                    mobileLayout:true,
                    schema:{
                        order:['type','url','action','text','align','background','color'],
                        fields:{
                            type:{
                                type:'text',
                                name:'Action Type',
    ...
    oncontextbar: function(e, contextbar)
    {
        ...
        var node = data.getComponent();
        var $el = $R.dom(node);
        if($el.attr('data-redactor-type')=='button'){
            var $point = $R.dom('<span>');
            var url = $el.attr('href');
            $point.text(this._truncateText(url));
            $point.attr('href', url);
            $point.attr('target', '_blank');
            ...
            if(type=='appaction'){
                var buttons = {
                    "action": {
                        title: 'Action: '+((action)?action:'no action')
                    },
                    "edit": {
                        title: this.lang.get('edit'),
                        api: 'plugin.button.open'
    ...
    {
        this.app.api('module.modal.close');
        this._setLinkData(this._getCurrent(), data, 'updated');
        this._resetCurrent();
    },
    _insert: function(data)
    {
    ...
    _removeInSelection: function(links)
    {
        ...
        for (var i = 0; i < links.length; i++)
        {
            var $link = $R.create('button.component', this.app, links[i]);
            var $clonedLink = $link.clone();
            $link.unwrap();

            // callback
            this.app.broadcast('button.deleted', $clonedLink);
        }
        this.selection.restore();
    },
    _insertMultiple: function(data)
    ...
    _insertSingle: function(links, data)
    {
        ...
        if (links.length === 1 && (links[0].textContext === this.selection.getText()) || (inline && inline.tagName === 'A'))
        {
            var $link = $R.create('button.component', this.app, links[0]);
            $link.setData(data);
            this.caret.setAfter($link);

            // callback
            this.app.broadcast('button.inserted', $link);
    ...
    _setLinkData: function(nodes, data, type)
    {
        ...
        if(nodes.length){
            for (var i = 0; i < nodes.length; i++)
            {
                var $link = $R.create('button.component', this.app, nodes[i]);
                var linkData = {};

                // if (data.text && isTextChanged) linkData.text = data.text;
                // if (data.url) linkData.url = data.url;
                // if (data.background) linkData.background = data.background;
                // if (data.color) linkData.color = data.color;
                // if (data.title !== undefined) linkData.title = data.title;
                // if (data.target !== undefined) linkData.target = data.target;
                $link.setData(data);

                // callback
                this.app.broadcast('button.' + type, $link);
            }
        }else{
            var $link = $R.create('button.component', this.app, nodes);
            var linkData = {};
    ...
    {
        var commands;
        if (this.$link.getData().url)//already created, editing now
        {
           commands = {
                update: { title: this.lang.get('save') },
                unlink: { title:'Remove', type: 'danger' },
                cancel: { title: this.lang.get('cancel') }
            };
        }
        else
    ...
    _buildCurrent: function()
    {
        ...
        var data = this.inspector.parse(current);
        var $link;
        $link = $R.create('button.component', this.app,data.getLink());
        var get=['url','text','background','color','align','action','type'];
        var linkData={};
        for (var i = 0; i < get.length; i++) {
            var name=get[i];
            linkData[name]=$(current).attr('data-'+name);
        }
        $link.setData(linkData);
        return $link;
    },
...
$R.add('class', 'button.component', {
    ...
    getData: function()
    {
        var names = ['url', 'text','background','color','align','action','type'];
        var data = {};
        ...
        if(!data.background) data.background='#194887';
    ...
    _init: function(el)
    {
        ...
        if (el === undefined)
        {
            this.$link = $R.dom('<a data-redactor-link="1">');
            this.$button=$R.dom('<div data-redactor-span="1" style="display:inline-block;margin:auto;padding:10px 20px;background:#194887;color:#FFFFFF">');
            this.parse('<figure>');
            this.$link.wrap(this.$button);
            this.append(this.$link);
        }
        else if ($figure.length === 0)
        {
            this.parse('<figure>');
            $el.css({textDecoration:'none',color:'#ffffff'}).attr('data-redactor-link',1);
            this.$link = $el;
            this.$button=$R.dom('<span data-redactor-span="1" style="display:inline-block;margin:auto;padding:10px 20px;background:#194887;color:#FFFFFF">');
            this.$link.wrap(this.$button);
            this.$button.wrap(this);
        }
        else
        {
            this.parse($figure);
            this.$link = this.find('a');
            this.$button =this.find('span')
        }
        this._initWrapper();
    },
    _ensure:function(){
        var data=this.getData();
        this.$link.attr('href',data.url).attr('target','_blank');
        this.attr('href',data.url);
        this.$link.html(data.text);
        var buttoncss={}
        buttoncss.backgroundColor=data.background;
        buttoncss.color=data.color;
        this.css({textAlign:data.align,'width':'100%','marginLeft':0,'marginRight':0})
        this.$button.css(buttoncss);
        this.$link.css({color:data.color})
        if(data.action){
            this.$link.attr('action','click:'+data.action)
            //this.$link.attr('data-action',data.action)
        }
    },
    _initWrapper: function()
    {
