modules.webrtc={
    ...
    call:function(room,user,video,debug){
        if(!debug){
            modules.api({
                caller:'Call',
                url: app.sapiurl+'/user/call/send/'+user.id,
                data:{
                    room:room,
                    video:(video)?1:'',
                ...
                callback:function(data){
                    if(data.success){
                        if(video) modules.webrtc.startVideo(room,user);
                        else modules.webrtc.startAudio(room,user);
                       	modules.webrtc.currentoptions={
                            user:user,
                            video:video
                        };
                    }else{
                        _alert('error starting call:'+data.error);
                    }
                }
            });
        }
    ...
    clearBG:function(){
        ...
        if(isPhoneGap()&&app.device=='iOS'){
            if(window.plugins.webviewcolor) window.plugins.webviewcolor.change('#00000000');
            $('html,body').css('background','transparent');
            $('#wrapper').hide();
            $('#footer_color').hide();
            modules.webrtc.ele.css('background','transparent');
            //hide all modals!
            ...
            modules.webrtc.ele.css('background','black')
        }
    },
    resetBG:function(){
        if(isPhoneGap()&&app.device=='iOS'){
            if(window.plugins.webviewcolor) window.plugins.webviewcolor.change('#FF000000');
    ...
    incoming:function(options){
        var self=this;
        if(!isPhoneGap()){
            if(modules.tools.isWebLayout()) modules.webrtc.initIncomingWeb(options);
            else modules.webrtc.initIncomingAndroid(options);
        }else if(app.device=='Android'){//should be avail asap!
            if(app.inForeground&&false){//do this directly
                modules.webrtc.initIncomingAndroid(options);
            }else{
                window.screenLocker.unlock(function(){
                    window.plugins.bringtofront();
                    modules.webrtc.initIncomingAndroid(options);
                }, function(){
                    onerror('error unlocking screen');
                })
            }
        }else{
            //modules.webrtc.initIncomingIOS(options);
            modules.webrtc.initIncomingAndroid(options);
        }
    },
    testCall:function(join){
        var self=this;
        if(!join){
        	modules.webrtc.startVideo(modules.webrtc.testroom,app.user.profile);
        }else{
	        modules.webrtc.initIncomingAndroid({
	            video:true,
	            room:modules.webrtc.testroom,
	            user:{
	                id:'UIAMPLAYER12',
	                name:'Tom Bassett',
	                pic:'https://s3.amazonaws.com/one-light/upload/cd9aa8ef94697d/square.jpeg'
	            }
	        })
	    }
    },
    initIncomingWeb:function(options){
        var self=this;
        $('#growlarea').render({
            template:'rtccall_web',
            data:$.extend(true,{},options),
            binding:function(ele){
                app.showProfile(options.user,'chat');
                ele.find('.x_decline').stap(function(){
                    modules.webrtc.decline();
                    ele.fadeOut(500,function(){
                        ele.remove();
                    })
               },1,'tapactive'); 
               ele.find('.x_accept').stap(function(){
                    setTimeout(function(){
                        ele.remove();
                    },1000)
                    if(options.video){
                       	modules.webrtc.startVideo(options.room,options.user,1);
                    }else{
                       	modules.webrtc.startAudio(options.room,options.user,1);
                    }
               },1,'tapactive'); 
            }
        })
    },
    initIncomingAndroid:function(options){
        var self=this;
        $('body').render({
            template:'rtccall_android',
            data:$.extend(true,{},options),
            binding:function(ele){
                ele.find('.x_decline').stap(function(){
                    modules.webrtc.decline();
    ...
    voipNumber:5555555555,
    removeContact:function(cb){
        // find all contacts
        var options = new ContactFindOptions();
        options.filter = modules.webrtc.voipNumber;
    ...
    clearContact:function(cb){
        var self=this;
        if(modules.webrtc.temp_image){//not used anymore
            app.core.photoLibrary.remove(modules.webrtc.temp_image,function(){
                _alert('cleared')
            },function(){
                _alert('error');
            });
        }
        if(modules.webrtc.current_object) modules.webrtc.current_object.remove(function(){
            if(cb) cb();
            modules.webrtc.current_object=false;
    ...
    ensureContact:function(options,cb){
        var self=this;
        if(isPhoneGap()&&app.device=='iOS'){
            modules.webrtc.saveTempContact(_.getImg(options.user.pic,'small'),options,cb);
            //not able to get working with fullscreen image in callkit calling...
            // app.core.photoLibrary.save(_.getImg(options.user.pic,'small'),function(uri){
            //     modules.webrtc.temp_image=uri;
            //     modules.webrtc.saveTempContact(uri,options,cb);
            // },function(er){
            //     modules.webrtc.saveTempContact(_.getImg(options.user.pic,'small'),options,cb);
            // })
        }
    },
    saveTempContact:function(url,options,cb){
        var self=this;
        var myContact = navigator.contacts.create({id:"nectar_voip","displayName": options.user.name});
        var phoneNumbers = [];
        phoneNumbers[0] = new ContactField('work', modules.webrtc.voipNumber, true);
        myContact.phoneNumbers = phoneNumbers;
        //save image to camera roll
        if(url.indexOf('http')>=0){
            myContact.photos=[new ContactField('url', url, true)];
        }else{
            myContact.photos=[new ContactField('base64', url, true)];
        }
        myContact.save(function (contact_obj) {
            modules.webrtc.current_object=contact_obj;
            cb();
        });
    },
    decline:function(){
        console.log('decline==todo')
    },
    initIncomingIOS:function(options){
        var self=this;
       	modules.webrtc.currentoptions=options;
        if(app.inForeground){
            modules.webrtc.initIncomingAndroid(options);
        }else{
            if(options.video){
                ...
                cordova.plugins.CordovaCall.setVideo(false);
            }
            modules.webrtc.ensureContact(options,function(){
                cordova.plugins.CordovaCall.receiveCall(options.user.name,modules.webrtc.voipNumber,function(){//within app??!
                   //  app.showProfile(options.user,'chat');
                   // if(options.video){
                   //     	modules.webrtc.startVideo(options.user,1);
                   //  }else{
                   //     	modules.webrtc.startAudio(options.user,1);
                   //  }
                });
            })
        }
        modules.webrtc.received=1;
    },
    stopVideo:function(stopped){
        var self=this;
        if(isPhoneGap()) window.plugins.insomnia.allowSleepAgain()
        modules.webrtc.endCall(function(){
        	if(modules.webrtc.connection){
	            if(!stopped) modules.webrtc.connection.close();
	            modules.webrtc.connection.attachStreams.forEach(function(stream) {
	                stream.stop();
	            });
	        }
            modules.webrtc.resetBG();
            modules.webrtc.ele.find('#videocontainer').find('.videos').remove();
            modules.webrtc.ele.fadeOut(500,function(){
                $(this).remove()
            });
        })
    },
    reject:function(){
        modules.webrtc.clearContact();
    },
    endCall:function(cb){
        var self=this;
        modules.webrtc.clearContact();
        if(isPhoneGap()&&app.device=='iOS'&&cordova.plugins.CordovaCall){
    ...
    stopAudio:function(stopped){
        ...
        modules.webrtc.resetBG();
        modules.webrtc.ele.find('#audiocontainer').find('.audios').remove();
        modules.webrtc.ele.fadeOut(500,function(){
            $(this).remove()
        })
        if(modules.webrtc.received&&cordova.plugins.CordovaCall){
            cordova.plugins.CordovaCall.endCall();
        }
    },
    attachAudioVisualizer:function(svg,stream){
        //https://codepen.io/zapplebee/pen/gbNbZE
    },
    startAudio:function(room,renderopts,add){
        var self=this;
        _.ensureAndroidPerms(['RECORD_AUDIO'],function(){
            $('body').render({
                template:'rtccall_audio',
                data:$.extend(true,{},{calling:(add)?0:1},renderopts),
                binding:function(ele){
                    modules.webrtc.ele=ele;
                    ele.stap(function(){
                        ele.toggleClass('showcontrols')
                    },1,'tapactive');
                    ele.find('.x_endcall').stap(function(){
                        modules.webrtc.stopAudio();
                    },1,'tapactive')
                }
            })
            modules.webrtc.connection = new RTCMultiConnection();
            modules.webrtc.connection.bandwidth = {
                audio: 510,  // 50 kbps
                video: 2000, // 256 kbps
                screen: 4000 // 300 kbps
            };
            modules.webrtc.connection.socketURL = app.siteurl+':3338/';
            modules.webrtc.connection.session = {
                audio: true
            };
            modules.webrtc.connection.sdpConstraints.mandatory = {
                OfferToReceiveAudio: true,
                OfferToReceiveVideo: false
            };
            modules.webrtc.connection.mediaConstraints = {
                audio: true,
                video: false
            };
            modules.webrtc.connection.onstream = function(event) {
                var sid=event.streamid;
                var cid=sid+'_container';
                event.mediaElement.controls=false;
                event.mediaElement.setAttribute('webkit-playsinline','webkit-playsinline')
                event.mediaElement.setAttribute('playsinline','playsinline');
                if(event.type=='remote'){//remote video!
                    var me=$(event.mediaElement);
                    me.css({width:'100%',height:'100%'})
                    if(isPhoneGap()) me.css({objectFit:'cover'});//objectFit:'cover'
                    me.attr('id', sid);
                    modules.webrtc.ele.find('#audiocontainer').append('<div style="position:absolute;top:0;left:0;right:0;bottom:0px;z-index:1;" id="'+cid+'" class="audios"></div>');
                    $('#'+cid).append(me);
                }else{//local video, start as full and then transition to
                    //dont show local video!
                    var me=$(event.mediaElement);
                    me.css({width:'100%',height:'100%',zIndex:2});//objectFit:'cover'
                    if(isPhoneGap()) me.css({objectFit:'cover'});//objectFit:'cover'
                    me.attr('id', sid);
                    modules.webrtc.ele.find('#audiocontainer').append('<div style="position:absolute;top:0;left:0;right:0;bottom:0;z-index:2" id="'+cid+'" class="audios"></div>')
                    modules.webrtc.ele.find('#'+cid).append(me);
                    setTimeout(function(){
                        modules.webrtc.ele.find('#audioloading').fadeOut(500,function(){
                            $(this).remove();
                        })
                    },10)
                }
            };
            modules.webrtc.connection.onopen = function() {
                alert('opened')
            };
            modules.webrtc.connection.onstreamended = function(event) {
                //growl user left!
                modules.webrtc.ele.find('#'+event.streamid+'_container').remove();
                if (modules.webrtc.connection.getAllParticipants().length==1){
                    modules.webrtc.connection.attachStreams.forEach(function(stream) {
                        stream.stop();
                    });
                    modules.webrtc.stopAudio(1)
                }
            };
            modules.webrtc.connection.onEntireSessionClosed = function(event) {
                modules.webrtc.connection.attachStreams.forEach(function(stream) {
                    stream.stop();
                });
                // don't display alert for moderator
                if (modules.webrtc.connection.userid === event.userid) return;
                //'Entire session has been closed by the moderator: ' + event.userid;
            };
            modules.webrtc.connection.onclose = function() {
                modules.webrtc.connection.attachStreams.forEach(function(stream) {
                    stream.stop();
                });
                modules.webrtc.stopAudio(1);
            ...
            if(add){
                modules.webrtc.connection.join(room);
            }else{
                modules.webrtc.connection.open(room);
            }
            //modules.webrtc.connection.openOrJoin(room);
        });
    },
    turnOnSpeaker:function(){
        // if(app.device=='iOS'){
        //     cordova.plugins.CordovaCall.speakerOn(function(){},function(){});
        // }else{
    ...
    turnOffSpeaker:function(){
        // if(app.device=='iOS'){
        //     cordova.plugins.CordovaCall.speakerOff(function(){},function(){});
        // }else{
	...
	startVideo:function(room,renderopts,add){
        var self=this;
        if(isPhoneGap()) window.plugins.insomnia.keepAwake()
        _.ensureAndroidPerms(['CAMERA','RECORD_AUDIO'],function(){
                $('body').render({
                    template:'rtccall',
                    data:$.extend(true,{},{calling:(add)?0:1},renderopts),
                    binding:function(ele){
    			...
    			modules.webrtc.connection.onstream = function(event) {
                    ...
                    if(event.type=='remote'){//remote video!
                        ...
                        if(app.device=='iOS'){
                            me.css({zIndex:-2})
                            modules.webrtc.ele.find('#videocontainer').append('<div style="position:absolute;top:0;left:0;right:0;bottom:00px;z-index:-2;" id="'+cid+'" class="videos"></div>');
                        }else{
                            modules.webrtc.ele.find('#videocontainer').append('<div style="position:absolute;top:0;left:0;right:0;bottom:00px;z-index:1;background:black;" id="'+cid+'" class="videos"></div>');
                        }
                        $('#'+cid).append(me);
                        setTimeout(function(){
                            var tele=modules.webrtc.ele.find('#'+modules.webrtc.localVideo.cid);
                            var ar=modules.webrtc.localVideo.event.mediaElement.videoWidth/modules.webrtc.localVideo.event.mediaElement.videoHeight;
                            var height=150;
                            var width=ar*height;
                            modules.webrtc.data={
                                width:width,
                            ...
                            tele.css({width:modules.webrtc.data.width+'px',height:modules.webrtc.data.height+'px',top:'10px',left:'10px'}).addClass('s-corner-all');
                            modules.webrtc.ele.find('#'+modules.webrtc.localVideo.sid).css({zIndex:2}).addClass('s-corner-all');
                            modules.webrtc.refreshVideos();
                            //make it dragable now!
                            Draggable.create(tele, {
                                type:"x,y",
                                bounds:{minX:0,maxX:$('body').width()-modules.webrtc.data.width-20,minY:0,maxY:$('body').height()-modules.webrtc.data.height-20},
                                throwProps:true,
                                force3D:true,
                                cursor:'defualt',
                                edgeResistance:1,
                        ...
                        //dont show local video!
                        var me=$(event.mediaElement);
                        me.css({width:'100%',height:'100%'});//objectFit:'cover'
                        if(isPhoneGap()) me.css({objectFit:'cover'});//objectFit:'cover'
                        me.attr('id', sid);
                        if(app.device=='iOS'){
                            me.css({zIndex:-2});
                            modules.webrtc.ele.find('#videocontainer').append('<div style="position:absolute;top:0px;left:0px;right:0px;bottom:00px;z-index:2" id="'+cid+'" class="videos"></div>')
                        }else{
                            me.css({zIndex:2});
                            modules.webrtc.ele.find('#videocontainer').append('<div style="position:absolute;top:0;left:0;right:0;bottom:0;z-index:2" id="'+cid+'" class="videos"></div>')
                        }
                        modules.webrtc.ele.find('#'+cid).append(me);
                        setTimeout(function(){
                            modules.webrtc.ele.find('#videoloading').fadeOut(500,function(){
                                $(this).remove();
                            })
                        },10)
                        modules.webrtc.localVideo={
                            event:event,
                            cid:cid,
                            sid:sid
                        }
                        modules.webrtc.turnOnSpeaker();
                        // setTimeout(function(){
                        //     var ar=event.mediaElement.videoWidth/event.mediaElement.videoHeight;
                        //     var height=150;
                        //     var width=ar*height;
                        //     modules.webrtc.data={
                        //         width:width,
                ...
                modules.webrtc.connection.onstreamended = function(event) {
                    ...
                    if (modules.webrtc.connection.getAllParticipants().length==1){
                        ...
                        modules.webrtc.stopVideo(1)
                    }
                };
                modules.webrtc.connection.onMediaError = function(e) {
                	console.warn(e)
                }
                modules.webrtc.connection.onEntireSessionClosed = function(event) {
                    modules.webrtc.connection.attachStreams.forEach(function(stream) {
    ...
    refreshVideos:function(){
        if(isPhoneGap()&&app.device=='iOS'&&cordova.plugins&&cordova.plugins.iosrtc){
