modules.service_checkout=function(options,instance){
	var oself=self=this;
	...
	this.reserve=function(cb){
		...
		modules.api({
			url:app.sapiurl+'/module/service_checkout/reserve',
			data:{
	...
	this.showLoading=function(cb){
		...
		rele.render({
			template:(options.template)?options.template:'service_checkout_page',
			data:options,
	...
	this.getReceipt=function(){
		...
		if(self.reserveResp.absorb_fees){
			...
			var pfee=modules.tools.calcPlatformFee(total,self.reserveResp.paltformFeeCalculation);
	...
	this.pages={
		...
		addCard:function(opts){
    		...
    		this.show=function(replace){
    			oself.ele.find('.pagepane').subpage({
		            loadtemplate:'service_checkout_addcard',
		            data:{
		            },
		...
		payment:{
			render:function(){
				...
				if(!pself.ele){
					self.ele.find('.pagepane').render({
						template:self.getTemplate('service_checkout_payment'),
						data:{
			...
			renderDonation:function(){
				...
				pself.ele.find('.donationarea').render({
					template:'service_checkout_add_donation',
					append:false,
					data:{
			...
			checkDiscount:function(){
				...
				modules.api({
					url:app.sapiurl+'/module/service_checkout/checkdiscount',
					data:{
						event:options.event.id,
						code:pself.code
					},
			...
			renderDiscount:function(){
				...
				pself.ele.find('.discountarea').render({
					template:'service_checkout_discount',
					append:false,
					data:{
						discount:(self.discount)?self.discount:''
					},
			...
			renderMethods:function(){
				...
				pself.ele.find('.methodarea').render({
					template:'service_checkout_methods',
					append:false,
					data:{
			...
			loadAnonMethods:function(){
				var pself=this;
				pself.ele.find('.methodarea').render({
					template:'service_checkout_anonmethods',
					append:false,
					data:{
					},
			...
			loadMethods:function(){
				...
				pself.ele.find('.methodarea').render({
					template:'service_checkout_methods_loading'
				})
				pself.loading=true;
				modules.api({
					url:app.sapiurl+'/module/service_checkout/methods',
					data:{
					},
					callback:function(resp){
						pself.loading=false;
			...
			sendPayment:function(cb,token){
				...
				modules.api({
					url:app.sapiurl+'/module/service_checkout/send',
					data:data,
					timeout:20000,
		...
		thanks:{
			render:function(){
				...
				if(!pself.ele){
					...
					self.ele.find('.pagepane').render({
						template:self.getTemplate('service_checkout_thanks'),
						data:{
