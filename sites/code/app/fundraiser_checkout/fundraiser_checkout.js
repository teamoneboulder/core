modules.fundraiser_checkout=function(options,instance){
	var oself=self=this;
	...
	this.reserve=function(cb){
		...
		modules.api({
			url:app.sapiurl+'/module/fundraiser_checkout/reserve',
			data:{
		...
		if(self.reserveResp.error){
			if(self.reserveResp.error=='ticket_sales_closed'){
				self.ele.find('.pagepane').render({
					template:'fundraiser_checkout_closed',
					append:false,
					data:{
	...
	this.showLoading=function(cb){
		...
		rele.render({
			template:(options.template)?options.template:'fundraiser_checkout_page',
			data:options,
	...
	this.isDonationBased=function(){
		if(options.contributions[0]&&options.contributions[0].data&&options.contributions[0].data.type=='donation') return true;
		return false;
	}
	this.getReceipt=function(){
		...
		if(options.donation){
			...
			$.each(options.contributions,function(i,v){
	...
	this.pages={
		cart:{
			render:function(){
				...
				if(!pself.ele){
					self.ele.find('.pagepane').render({
						template:self.getTemplate('fundraiser_checkout_cart'),
						data:{
							contributions:options.contributions,
		...
		addCard:function(opts){
    		...
    		this.show=function(replace){
    			oself.ele.find('.pagepane').subpage({
		            loadtemplate:'fundraiser_checkout_addcard',
		            data:{
		            },
    	...
    	login:{
    		render:function(){
				...
				if(!pself.ele){
					self.ele.find('.pagepane').render({
						template:self.getTemplate('fundraiser_checkout_login'),
						data:{
						...
						binding:function(ele){
							...
							pself.form=new modules.formbuilder({
                                ...
                                current:{
                                	fid:options.fundraiser.id,
                                	referal:options.fundraiser.page
                                },//passed as a refernce
                                schema:self.reserveResp.fundraiser_anon,
                                inline:true,
                            ...
                            ele.find('.x_create').stap(function(){
                            	if(isPhoneGap()||!_.isWebLayout()){
                            		phi.registerView('onboard',{//always do this!
										renderTo:$('body')[0],
										showOptions:{
											display:'page_overlay'
										},
										onComplete:function(){
											//reload this!!!
											self.show(1);
		...
		payment:{
			render:function(){
				...
				if(!pself.ele){
					self.ele.find('.pagepane').render({
						template:self.getTemplate('fundraiser_checkout_payment'),
						data:{
			...
			renderDonation:function(){
				...
				pself.ele.find('.donationarea').render({
					template:'fundraiser_checkout_add_donation',
					append:false,
					data:{
			...
			checkDiscount:function(){
				...
				modules.api({
					url:app.sapiurl+'/module/fundraiser_checkout/checkdiscount',
					data:{
						fundraiser:options.fundraiser.id,
						code:pself.code
					},
			...
			renderDiscount:function(){
				...
				pself.ele.find('.discountarea').render({
					template:'fundraiser_checkout_discount',
					append:false,
					data:{
						discount:(self.discount)?self.discount:''
					},
			...
			renderMethods:function(){
				...
				pself.ele.find('.methodarea').render({
					template:'fundraiser_checkout_methods',
					append:false,
					data:{
			...
			loadAnonMethods:function(){
				var pself=this;
				pself.ele.find('.methodarea').render({
					template:'fundraiser_checkout_anonmethods',
					append:false,
					data:{
					},
			...
			loadMethods:function(){
				...
				pself.ele.find('.methodarea').render({
					template:'fundraiser_checkout_methods_loading'
				})
				pself.loading=true;
				modules.api({
					url:app.sapiurl+'/module/fundraiser_checkout/methods',
					data:{
					},
					callback:function(resp){
						pself.loading=false;
			...
			sendPayment:function(cb,token){
				...
				modules.api({
					url:app.sapiurl+'/module/fundraiser_checkout/send',
					data:data,
					timeout:20000,
		...
		donation:{
			render:function(){
				...
				if(!pself.ele){
					self.ele.find('.pagepane').render({
						template:'fundraiser_checkout_donation',
						data:{
		...
		thanks:{
			render:function(){
				...
				if(!pself.ele){
					...
					self.ele.find('.pagepane').render({
						template:self.getTemplate('fundraiser_checkout_thanks'),
						data:{
