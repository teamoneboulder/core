var app={
    ...
    donate:function(e,container,target,data){
        app.checkout=new modules.ticket_checkout($.extend(true,{},{
            tickets:[{
                id:data.id,
                data:app.getTicketById(data.id),
                quantity:1
            }],
            mode:'donation',
            ...
            //     if(anon){
            //         if(self.eventfeed) self.eventfeed.addPost(post_resp.data);
            //     }
            // },
            onSuccess:function(data){
                //re-render things!
                app.purchased=true;
                var resp=data.eventdata;
                // self.ele.find('.rsvps').render({
                //     template:(_.isWebLayout())?'event_rsvps_web':'event_rsvps',
                //     replace:true,
                //     data:{
                //         resp:resp,
                //         rsvp:resp.data.rsvp,
                //         id:resp.data.id,
                //         event:resp.data,
                //         tickets:(resp.data.tickets)?resp.data.tickets:false
                //     },
                //     binding:function(){
                //         self.bindRSVPS();
                //     }
    ...
    goToApp:function(){
        ...
        var url=(app.scheme+urlp[0]).replace('///','//').replace('tickets','event');
